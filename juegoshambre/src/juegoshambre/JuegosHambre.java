package juegoshambre;

import java.util.Random;
import java.util.Scanner;

public class JuegosHambre {
	public static void main(String[] args) {
		...
		// Escoger personaje, bucle while para seguir pidiendo en caso que el valor no
		// entre en el rago pedido
		...
		// switch para seleccionar la ruta inicial, Do while usado para seguir pidiendo
		// el valor en caso que valor dado por el usuario no entre en las
		// especificaiones dadas, ademas un uso del bloque try catch para evitar
		// excepciones en caso que se meta otro tipo de dato
		...
		case 1:
			...
			// Primera decision
			// Try Catch para evitar excepciones en caso de tipo de dato erroneo por parte
			// ususario
			try {
			...
			// SI no se completa el anagrama, no hay historia ya que no te presentas como
			// voluntario
			if (!voluntario) {
			...
			System.out.println(
					"Entras en la habitacion para mostrar habilidades por lo que coges tu arco. Clavas tu flecha en el centro de la diana tres veces pero ves que ningun rico te esta prestando atencion, que haces?");
			...
			eleccion = pedirNum(1, 2);
			// If separando la opciones que podias tomar en la historia
			if (eleccion == 1) {
				System.out.println(
						"Te vas, mostrando buena tecnica y fuerza. Los participantes te han prstando atencion, pero los patrocinadores no saben ni quien eres.");
			} else {
				System.out.println(
						"Disparas la flecha hacia la sala, golpea rompiendo una lampara,y echando cristales a la comida.\n Te miran con desprecio, YA SABEN QUIEN ERES, ya tienes algo positivo y algo negativo");
			}
			...
			System.out.println(
					" Suena la bocina casi inmediatamente. Observas un  arco brillando y te planteas ir a por el pero Gloss ya esta yendo hacia el. ");
			System.out.println(
					"Notas un mensaje brillante bajo el agua: \"El camino más rápido es el que nadie pisa, el reflejo marca la piedra segura\".\n Ves unas piedras que sobresalen y otras sumergidas que se ven por el reflejo.");
			// Pedida de valor a usuario
			// Bulce hasta que el valor introducido sea valido
			...
			System.out.println(
					"Tras la situacion de la cornucopia decides esconderte en el bosque para sobrevivir. \n Escuchas un ruido tras unos arboles. ");
			// Pedida de valor a usuario
			// Bulce hasta que el valor introducido sea valido
			...
			if (eleccion == 1) {
				System.out.println(
						"El ruido lo provocaba otra persona WIRESS, quien esta muy nerviosa y repite sin parar tic-tac tic-tac \n Para calmarla y conseguir información: ENCUENTRO CON WIRESS — “APUESTA DE MANOS”");
				...
				eleccion = pedirNum(1, 3);

				// Sucesion de if y else if para la division de camino dependiendo de lo
				// escogido en el piedra papel tijera

				if (PiePapTij(eleccion))
					tener_reloj = true;
				else
					tener_reloj = false;
			}
				...
				if (!tener_reloj) {
					System.out.println(
							"Estas descansando, tranquila, crees que nada puede salir mal, ESE SERA TU ULTIMO ERROR ");
					System.out.println(
							"Empiezas a escuchar ruidos de animales, estan sufriendo, pero no sabes porque, hasta que es demasiado tarde");
					System.out.println(
							"Una humareda blanca te ha rodeado, NO TIENES DONDE HUIR, empiezas a notas que te cuesta respirar y empiezas a toser sangre");
				...
				System.out.println(
						"EL sol cae, tu campo visual se esta reduciendo, notas que el entorno vibra, hasta que aparece la razon \n Un mono mutante alfa");
				...
				while (!combate_finalizado) {
					// If decidiendo cual es el turno actual mediante un booleano, (el turno inicial
					// se hace mediante un ran.nextboolean())
					if (turno) {
						...
						eleccion = pedirNum(1, 3);
						// Segun la eleccion de usuario, selecciona un movimiento, cada uno tiene su
						// daño establecido
						...
						// Random utilizado para esocger el movimiento del mono, cada uno tabmien tiene
						// su daño
						if (ran.nextInt(1, 4) == 1) {
				...
				if (eleccion == 1) {
					System.out.println(
							"Beetee niega lentamente. La arena no conduce electricidad, no hay tiempo para pensar mas");
					System.out.println("PLAN FALLIDO");
					plan = false;
				} else {
					System.out
							.println("Beetee sonríe satisfecho. El agua salada es la clave. El plan puede funcionar.");
					plan = true;
				}
				...
				if (!tener_arco) {
					...
					System.out.println(
							"Te das cuenta que el plan le falta algo. TU ARCO, no pueden ganar asi..... pero TU SI");
		...
		case 2:
			...
			System.out.println(
					"Entras en la habitacion para mostrar habilidades, decides intentar practicar con armas pero no es tu fuerte por lo que practicas camuflage. Piensas que hacer aliados será útil pero no estas seguro si podras conseguirlo");
			...
			if (eleccion == 1) {
				...
				System.out.println(
						"Te vas sin dejar marca en nadie, nadie te tiene como objetivo pero tampoco creen que eres util");
			}
			...
			System.out
					.println("—Dame una excusa para no matarte —dice—.\n Y que no sea “Katniss”. Estoy harta de ella");
			...
			while (!combate_finalizado) {
				...
				// el Turno inicial fue mediante la creacion de la variable con un
				// ran.nextboolean()
				if (turno) {
		...
		case 3:
			...
			// try catch para comprobar el tipo de valor
			System.out.println(
					"Entras en la habitacion para mostrar habilidades, caminas recto y con cara confiado, conoces bien las habilidades que posees. Piensas en dar un espectaculo");
			...
			if (eleccion == 1) {
				System.out.println(
						"Lanzas el tridente hacia un maniquin (se parece a PEETA), le arranca la cabeza de un solo golpe y ries en alto");
			} else {
			...
			pulso = PiePapTij(eleccion);
			System.out.println(
					"Tras tu situacion delicada con Chaff sigues moviendote, atento a cualquier cosa que te pueda dar una ventaja");
	...
	public static int pedirNum(int num1, int num2) {
		int eleccion = 0;
		Scanner sc = new Scanner(System.in);
		do {
			System.out.println("Pulse su eleccion");
			try {
				eleccion = sc.nextInt();
				sc.nextLine();
			} catch (Exception e) {
	...
	public static boolean PiePapTij(int elec) {
		Random ran = new Random();
		int ppt_maquina;
		ppt_maquina = ran.nextInt(1, 4);
		// COndicional para los casos del piedra papel y tijera
		if (elec == 1 && ppt_maquina == 1) {
			System.out.println("EMPATE");
			return true;

		} else if (elec == 1 && ppt_maquina == 2) {
			System.out.println("PIERDES");
			return false;

		} else if (elec == 1 && ppt_maquina == 3) {
			System.out.println("GANAS");
			return true;

		} else if (elec == 2 && ppt_maquina == 1) {
			System.out.println("GANAS");
			return true;

		} else if (elec == 2 && ppt_maquina == 2) {
			System.out.println("EMPATE:");
			return true;

		} else if (elec == 2 && ppt_maquina == 3) {
			System.out.println("PIERDES");
			return false;

		} else if (elec == 3 && ppt_maquina == 1) {
			System.out.println("PIERDES");
			return false;

		} else if (elec == 3 && ppt_maquina == 2) {
			System.out.println("GANAS");
			return true;

		} else if (elec == 3 && ppt_maquina == 3) {
			System.out.println("EMPATE");
			return true;
